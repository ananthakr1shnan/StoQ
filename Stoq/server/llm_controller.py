...
def run_chatbot(message, chat_history=[]):
    print("Using Groq to generate response...")
    ...
    if search_queries:
        ...
        response = groq_client.chat.completions.create(
            model="llama3-groq-70b-8192-tool-use-preview",  # Using the 70B model for better performance
            messages=messages,
            temperature=0.7,
            max_tokens=4096
        )
...
# Initialize vectorstore with example data
raw_documents = []
print("Getting filings for NVDA")
raw_documents += ir.get_all_filings("NVDA")
print("Getting Benzinga news for NVDA")
raw_documents += ir.get_benzinga_news("NVDA")
print("Getting Yahoo news for NVDA")
raw_documents += ir.get_yahoo_news("NVDA")
vectorstore = Vectorstore(raw_documents=raw_documents)
